optimize_parameters <- function(time,y,sd,initial_value,model_spec,cov_spec,extra_cov_spec,compute_settings,plot_out_spec,fix_pred,max_values,mean_spec){
  ...
  while ((count < compute_settings$min_iter | log_like_diff > compute_settings$log_like_tol) &
    ...
    if (opt.mean){
      ...
      if (length(key_planet_paras_index) > 0){
        if (mean_spec$planet_period_search){
          period_out <- opt_period_batch_reuse_mats("mean_paras",basic_period_opt,key_planet_paras_index,planet_period_index,log_per_guesses,log_normalizing,shrink,plot_out_spec,mean_paras_bounds,for_like)
          for_like <- period_out[[1]]
        } else {
          for_like <- opt_batch_reuse_mats("mean_paras",key_planet_paras_index,plot_out_spec,mean_paras_bounds,for_like,log_normalizing,shrink)
        }
      }
  ...
  return(list(model_mat=model_mat,cov_paras=for_like$cov_paras,extra_cov_paras=for_like$extra_cov_paras,mean_paras=mean_paras,log_nugget_vec=for_like$log_nugget_vec,log_like_store=as.matrix(store_log_like),max_err_location=max_err_location,max_err=max_err,rv_errs=rv_errs,for_like=for_like))
