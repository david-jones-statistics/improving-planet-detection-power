...
output_name <- "gpca_hd"
home <- getwd()
data_home <- paste(home,"/final data/",basis_type,"/",output_name,"_null_datasets/",sep="")
setwd(home)
...
for (analysis_loop in 1:2){
  ...
  for (data_loop in 1:data_num){
    ...
    for (initial_loop in 1:num_initializations[analysis_loop]){
      ...
      plot.data <- FALSE
      make_cov_mat <- FALSE
      data <- get_data(paste(data_home,output_name,"_dataset",jid,".RData",sep=""),n,num_outputs,missing.data.index.store[[data_loop]],plot.data,make_cov_mat)
...
save(list=c("gpca_max_signal","for_like","time_taken","missing.data.index.store","data","model_spec","mean_spec","output_by_model_data_initial","max_log_like_by_model_data_initial","initial_value_by_model_data_initial","plot_out_spec"),file=paste(home,"/output/",basis_type,"_",output_name,"_",model_name,"_null_all_dataset_",jid,".RData",sep=""))
